//  Converts Binary and Decimal numbers
//  Hunter Crisp
//  8/23/2021
...
int binary_to_decimal(string s){
    ...
    //Using two iterators, use power of 2 for each 1 found
    for (int i = s.length() - 1; i >= 0; i--)
    {
        ...
        //Increase iterator to increase the power of 2
        t++;
    }
...
string decimal_to_binary(int n){
    ...
    {
        //While not 0, take remainder of n/2, either 0 or 1, and add to beginning of string
        while (n != 0)
        {
            string temp = to_string(n % 2);
            binary.insert(0, temp);

            //Divide value of n by 2
            n /= 2;
        }
    }
...
string add_binaries(string b1, string b2){
    //Convert to decimal, add, convert to binary
    int a1 = binary_to_decimal(b1);
    int a2 = binary_to_decimal(b2);
    return decimal_to_binary(a1 + a2);
}
