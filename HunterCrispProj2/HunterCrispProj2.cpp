...
string signed_extension(string s) {
    ...
    //If leading digit is 0, add '0' to the front until length is 16
    if (s[0] == '0')
    {
        s = string(16 - s.length(), '0') + s;
    }
    //Else, add '1' to the front until length is 16
    else
    {
        s = string(16 - s.length(), '1') + s;
    }
    return s;
}
...
int binary_to_decimal_signed(string s) {
    ...
    //Look at leading bit to determine if positive or negative
    if (s[0] == '0')
    {
        //Positive
...
string decimal_to_binary_signed(int n) {
    ...
    {
        string invN = decimal_to_binary(-n);
        if (invN == "1")
        {
            //Return -1
            return signed_extension("1");
        }
        else
        {
            //Return negative binary of positive number
            return twos_complement("0" + invN);
        }
    }
}
...
string add_binaries_signed(string b1, string b2) {
    ...
    //Convert both to signed decimals
    int a1 = binary_to_decimal_signed(b1);
    int a2 = binary_to_decimal_signed(b2);
    //Convert sum to signed binary
    return decimal_to_binary_signed(a1 + a2);
}
...
string twos_complement(string s) {
    ...
    if (s == "0")
    {
        return signed_extension(s);
    }

    string sInv = s;
    //Make a string that is the inverse of s
    ...
    //Extend, add 1, and return
    return add_binaries(signed_extension(sInv), "1");
}
